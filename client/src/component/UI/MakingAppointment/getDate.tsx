...
let intermediate: Date = new Date(date.getFullYear(), date.getMonth(), intermediate_date)
...
export const Days = () => {
    ...
    useEffect(() => {
        ...
        let first_time: HTMLElement | null = document.querySelector('.making_appointment_timing_block')
        first_time!.classList.add('active')

        setTimeout(() => {
            day_button(`timing_block1`, intermediate.getDate(), intermediate.getMonth(), intermediate.getFullYear())
            setDisabled(false)
        }, 1000)
    }, [])
    ...
    useEffect(() => {
        ...
        time_segmented = []
        let today: Date = new Date(date.getFullYear(), date.getMonth(), (date.getDate()))
    ...
    for(let i = 1; i < 31; i++) {
        ...
        arr_data.push({day: getDate(intermediate), number: intermediate.getDate()})

        arr_dom.push(
            <div className='making_appointment_timing_block' id={"timing_block" + i} 
            key={'making_appointment_timing_block' + i} 
            onClick={() => day_button(`timing_block${i}`, intermediate.getDate(), intermediate.getMonth(), intermediate.getFullYear())}>
    ...
    for(let i = 0; i < 6; i++) {
        arr_time.push(
            <button id={'timing_btn' + i} className={'appointment_timing_right_btn ' + activeBtn[i]} key={'timing_btn' + i} disabled={disabled}
            onClick={(e) => timing_btn(e, `timing_btn${i}`, arr[i].hours, arr[i].minutes)}>{arr[i].hours + ':' + arr[i].minutes_render}</button>
        )
    }


    async function day_button (id: string, day: number, month: number, year: number) {
        state_variable = ['', '', '', '', '', '']
        ...
        let active_day_button = document.querySelectorAll('.making_appointment_timing_block')
    ...
    return (
        <div className="making_appointment_block">
            <div className='making_appointment_timing'>
                {arr_dom}
            </div>

            <div>
                <div className='appointment_timing_right'>
                    {arr_time}
                </div>

                <button className="appointment_timing_right_end" disabled={disabled} onClick={() => registration_visit(request)}>
...
function registration_visit(request: any) {
    let date: Date = new Date((visiting_time_obj.year || 22), visiting_time_obj.month, 
        visiting_time_obj.day, visiting_time_obj.hours, visiting_time_obj.minutes)
    ...
    request('/api/reception_unemployed/session_creation', 'POST', {date, localStr_id})

    alert('Данное время зарезервировано для посещеия и будет отображено в вашем личном кабинете')
