...
const AcceptanceSheet = () => {
	...
	useEffect(() => {
		let data = request('/api/reception_unemployed/time', 'POST').then(
			(value) => {
				return value
			}
		)
		...
		const data_acquisition = async () => {
			...
			let user = request('/api/users/user_search', 'POST', {
				userId: data_obj[0].unemployed,
			}).then((value) => {
				return value
			})
					...
					<div className='personal_area_admin_card'>
						...
						<div className='pa_main_information'>
							...
							{post.salary === undefined ? (
								...
								<div className='pa_field'>
									<p>Желаемая заработная плата:&ensp;</p>
							...
							{post.contacts === undefined ? (
								...
								<div className='pa_field'>
									<p>Дополнительные контакты:&ensp;</p>
