...
const ListEmployees = (props: {
	...
	return (
		<div>
			{element_display && (
				<>
					{index === 0 && <h1 className='title'>Список откликнувшихся</h1>}

					<div className='list_employees'>
						{value.profession !== undefined && (
							<h1 className='listVacancies_profession'>{value.profession}</h1>
						)}

						{value.name !== undefined && (
							<h1 className='listVacancies_company'>{value.name}</h1>
						)}

						{value.salary !== undefined && (
							<h1 className='listVacancies_salary'>{value.salary} руб.</h1>
						)}
